...
# 38 "FID_class.h" 2
# 1 "/usr/include/c++/4.8.2/cstdarg" 1 3
# 39 "/usr/include/c++/4.8.2/cstdarg" 3
       
# 40 "/usr/include/c++/4.8.2/cstdarg" 3


# 1 "/usr/lib/gcc/x86_64-redhat-linux/4.8.5/include/stdarg.h" 1 3 4
# 43 "/usr/include/c++/4.8.2/cstdarg" 2 3
# 52 "/usr/include/c++/4.8.2/cstdarg" 3
namespace std
{
  using ::va_list;
}
...
# 8 "../Share_Headers.h" 2
# 1 "/usr/include/c++/4.8.2/cstdbool" 1 3
# 29 "/usr/include/c++/4.8.2/cstdbool" 3
       
# 30 "/usr/include/c++/4.8.2/cstdbool" 3
# 39 "/usr/include/c++/4.8.2/cstdbool" 3
# 1 "/usr/lib/gcc/x86_64-redhat-linux/4.8.5/include/stdbool.h" 1 3 4
# 40 "/usr/include/c++/4.8.2/cstdbool" 2 3
# 9 "../Share_Headers.h" 2
...
# 11 "../Share_Headers.h" 2
# 1 "/usr/include/c++/4.8.2/cstdarg" 1 3
# 39 "/usr/include/c++/4.8.2/cstdarg" 3
       
# 40 "/usr/include/c++/4.8.2/cstdarg" 3
# 12 "../Share_Headers.h" 2
...
# 13 "../Share_Headers.h" 2


# 1 "../_Copy_String_.h" 1
# 19 "../_Copy_String_.h"
 void _Copy_String_(char *,const char *,const size_t);
# 16 "../Share_Headers.h" 2
# 44 "FID_class.h" 2
...
namespace FID{
 ...
 class FID_class{
  ...
  private:
   ...
   DIR* Directory;
   DIL* List_Head;
   int Current_Length_List;



   int* FDES_OPENED;
   int Current_Length_FDES;
  ...
  public:
   ...
   bool State_Of_Initialization_FID;

   int _Had_Opened_(void)
   {
    return Current_Length_FDES;
   }

   int _Had_Linked_(void)
   {
    return Current_Length_List;
   }




   int _OPEN_(const char* FILENAME,int FLAG,...);
   void _CLOSE_(const int CLOSE_DES);
   void _UNLINK_(const char* UNLINK_DES);
   int _MKSTEMP_(char* TEMPFILE);
   int _LSEEK_(const int SEEK_DES,const off_t NEW_POS,const int WHENCE);

   int _LINK_(const char* OLD,const char* NEW);

   ssize_t _READ_FILE_(const int FDES,void* BUFF,const size_t LEN);

   ssize_t _WRITE_FILE_(const int FDES,const void* BUFF,const size_t LEN);

   struct stat _LSTAT_(const char* FILENAME);

   int _RENAME_(const char *OLD_NAME,const char *NEW_NAME);

   int _FTRUNCATE_(const int FDES,const off_t LENGTH);



   int _LOCK_FILE_(const int LOCKFDES,const int CMD,const struct flock* LOCK);





   bool _OPEN_DIR_(const char* DIRPATH);
   void _CLOSE_DIR_(void);

   struct dirent* _READ_DIR_(void);


   void _REWIND_DIR_(void);

   bool _CHDIR_(const char* New_Directory);

   void _GETCWD_(char* Buffer,const size_t Buffer_Len);


   int _MAKE_DIR_LIST_(void);

   void _TRAVERSE_(char* DNAME);
   void _CLEAR_LIST_(DIL* NODE);
   void _CLEAR_LIST_();
   ...
   short int _ROOTHOME_(char *Path_Array,const size_t Array_Len)
   {
    if (__null != Path_Array)
    {
     ...
     __asm__ volatile (
       ...
       "popq	%%rsi\n\t"
       :
       : "r" (Root_Home),"r" (Path_Array),"r" (Array_Len-1)
       : "%rsi","%rdi","%rcx"
      );
     Path_Array[Array_Len]='\0';
     return 1;
    }
    else
     return -1;
...
namespace FID{
 ...
 FID_class::FID_class():Directory(__null),List_Head(__null),Current_Length_List(0),Current_Length_FDES(0)
 {



  State_Of_Initialization_FID=true;



  _PATH_MAX_=pathconf(".",_PC_PATH_MAX);
  _NAME_MAX_=pathconf(".",_PC_NAME_MAX);

  _OPEN_MAX_=sysconf(_SC_OPEN_MAX);
  ...
  if (! Work_Home)
  {
   ...
   syslog(3|(1<<3),"Can not get memory - %d",1);
  }
  else;
  ...
  if (! Root_Home)
  {
   delete[] Work_Home;
   State_Of_Initialization_FID=false;
   syslog(3|(1<<3),"Can not get memory - %d",2);
  }
  else;
  ...
  if (! FDES_OPENED)
  {
   ...
   delete[] Root_Home;
   State_Of_Initialization_FID=false;
   syslog(3|(1<<3),"Can not get memory - %d",3);
  }
  else;
  ...
  if (State_Of_Initialization_FID)
  {
   ...
   if (FDES_OPENED)
   {
    for (int Index=0; Index < Current_Length_FDES; ++Index)
     (void)_CLOSE_(FDES_OPENED[Index]);
    delete[] FDES_OPENED;
   }
   else;
   ...
   syslog(5|(1<<3),"FID_class had been deleted! - %ld",getpid());
  }
  else
   syslog(5|(1<<3),"FID_class did nothing!");
 ...
 int FID_class::_OPEN_(const char* FILENAME,int FLAG,...)
 {
  ...
  if (FLAG & 0100)
  {
   ...
   va_list MODE;
   __builtin_va_start(MODE,FLAG);
   mode_t File_Mode=__builtin_va_arg(MODE,mode_t);
   __builtin_va_end(MODE);

   Tempolarly_Fdes=open(FILENAME,FLAG,File_Mode);
  ...
  else
  {
   ...
   Tempolarly_Fdes=open(FILENAME,FLAG);
   if (Tempolarly_Fdes < 0)
    return -1;
   else
    FDES_OPENED[Current_Length_FDES++]=Tempolarly_Fdes;
 ...
 {
  return lseek(SEEK_DES,NEW_POS,WHENCE);
 }
 ...
 struct stat FID_class::_LSTAT_(const char* FILENAME)
 {
  ...
  else
  {
   ...
   if ((Stat_BUF.st_mode & 0170000) == 0120000)
    Stat_BUF.st_size=0;
   else;
  }
 ...
 int FID_class::_LOCK_FILE_(const int LOCKFDES,const int CMD,const struct flock* LOCK)
 {
  ...
  switch (CMD)
  {
   case 5:
    ...
    return fcntl(LOCKFDES,5,LOCK);

   case 6:

    return fcntl(LOCKFDES,6,LOCK);

   case 7:

    return fcntl(LOCKFDES,7,LOCK);
 ...
 bool FID_class::_CHDIR_(const char* New_Directory)
 {
  ...
  else
  {
   ...
   _CLEAR_LIST_();
   Current_Length_List=_MAKE_DIR_LIST_();
 ...
 {
  const int The_Finally_Len=(Buffer_Len < _PATH_MAX_)?Buffer_Len:_PATH_MAX_;
 ...
 int FID_class::_MAKE_DIR_LIST_(void)
 {
  ...
  struct dirent* DIR_BUF(__null);
  List_Head=new DIL;
  if (List_Head)
  {
   List_Head->Next=__null;
   List_Head->Item_Name=__null;
  }
  else
   return -1;
  ...
  {
   if (*DIR_BUF->d_name == '.' && (*(DIR_BUF->d_name+1) == '.' || *(DIR_BUF->d_name+1) == '\0'))
    continue;
   else
   {
    ...
    Temp_ptr->Item_Name=new char[_NAME_MAX_];
    ...
    Temp_ptr->Next=new DIL;
    if (Temp_ptr->Next)
    {
     Temp_ptr=Temp_ptr->Next;


     Temp_ptr->Next=__null;
     Temp_ptr->Item_Name=__null;
    }
    else
     return Links;
 ...
 {
  static DIL* Next_Pos(List_Head);
  static short int Node_Num(0);

  if (Next_Pos && Node_Num < Current_Length_List)
  {
 ...
 inline void FID_class::_CLEAR_LIST_()
 {
 ...
 void FID_class::_CLEAR_LIST_(DIL* NODE)
 {
  if (NODE)
  {
   _CLEAR_LIST_(NODE->Next);
   delete[] NODE->Item_Name;
   delete NODE;
  }
  else
   ;
